...
def hazard_filters(args):
    ...
    if green_space_id:
        filters["green_space_id"] = green_space_id
    for key, group_key in (("status", "hazard_status"), ("risk_level", "hazard_risk_level"),
                           ("hazard_type", "hazard_type"), ("source", "hazard_source")):
        value = _enum(args, key, group_key)
        if value:
            filters[key] = value
    keyword = _text(args, "keyword")
    ...
    filters["date_from"] = _date(args, "date_from")
    filters["date_to"] = _date(args, "date_to")
    filters["open_only"] = _flag(args, "open_only")
    return filters
